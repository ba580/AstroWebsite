//${process.env.API_URL}/infraction?auth=${process.env.API_AUTH}/server?=req.query.id&id={singleid}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const infraction = await fetch(
      `${process.env.API_URL}/infraction?auth=${process.env.API_AUTH}&server=${req.query.id}&id=${req.query.singleid}`,
      {
        method: "GET",
        headers: {
